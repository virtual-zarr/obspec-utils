...
- Data chunks (from VirtualiZarr manifest)
...
def extract_chunks_dataframe(vdt) -> pd.DataFrame:
    """Extract all chunks from a virtual datatree into a DataFrame."""
    ...
    process_node(vdt.to_dataset(), "")
    for path, node in vdt.subtree:
        if path != "/":
            process_node(node.to_dataset(), path)

    return pd.DataFrame(records)


def compute_gaps(chunks_df: pd.DataFrame, file_size: int) -> list[dict]:
    """Compute gaps between chunks (metadata regions)."""
    if chunks_df.empty:
        return [
            {
                "x0": 0,
            ...
            gaps.append(
                {
                    ...
                    "description": f"Metadata: {current_pos:,} - {chunk_start:,}",
                }
            )
        ...
        gaps.append(
            {
                ...
                "description": f"Metadata: {current_pos:,} - {file_size:,}",
            }
        )
...
def visualize_layout(
    ...
    # Metadata gaps
    gaps = compute_gaps(chunks_df, file_size)
    gap_rects = [
        {
            "x0": gap["x0"] / MB,
            "x1": gap["x1"] / MB,
    ...
    gap_plot = hv.Rectangles(
        gap_df,
        kdims=["x0", "y0", "x1", "y1"],
        vdims=["source", "length_mb", "description"],
    ).opts(
        color="orange",
        alpha=0.8,
    ...
    open_vds_str = f"{open_vds_time:.2f} s" if open_vds_time else "N/A"
    ...
    stats_div = hv.Div(f"""
        <div style="background: #f8f9fa; border: 1px solid #dee2e6; border-radius: 4px;
            ...
            <br>
            <span style="color: darkorange;"><b>Metadata Regions:</b></span><br>
            &nbsp;&nbsp;Count: {len(gaps):,}<br>
            &nbsp;&nbsp;Size: {total_gap_bytes / MB:.2f} MB ({gap_pct:.1f}%)<br>
            <br>
            <b>Requests Made:</b> {len(requests_df):,}<br>
            <b>Bytes Requested:</b> {total_request_bytes / MB:.2f} MB<br>
            <b>open_virtual_datatree:</b> {open_vds_str}
        </div>
    """)
...
def main():
    ...
    # Open virtual datatree
    print("\n[4/5] Opening virtual datatree...")
    ...
    open_vds_time = time.perf_counter() - start_time

    print(f"      Requests: {trace.total_requests}")
    print(f"      Bytes requested: {trace.total_bytes / MB:.2f} MB")
    print(f"      Time: {open_vds_time:.2f}s")

    # Extract chunk info
    chunks_df = extract_chunks_dataframe(vdt)
    ...
    print(
        f"Metadata:        {total_gap_bytes / MB:>10.2f} MB ({100 * total_gap_bytes / file_size:.1f}%)"
    )
    print(
