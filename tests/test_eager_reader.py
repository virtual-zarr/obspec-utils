...
def test_eager_reader_with_request_size_and_file_size():
    ...
    mock_store = MockReadableStore(data)
...
def test_eager_reader_uses_head():
    ...
    summary = trace.summary()
    assert summary["total_requests"] == 5  # 1 head + 4 get_ranges
    assert trace.requests[0].method == "head"
    assert all(r.method == "get_ranges" for r in trace.requests[1:])
    assert summary["total_bytes"] == len(data)
...
def test_eager_reader_small_file_uses_single_get():
    ...
    summary = trace.summary()
    assert summary["total_requests"] == 2  # 1 head + 1 get
    assert trace.requests[0].method == "head"
    assert trace.requests[1].method == "get"
