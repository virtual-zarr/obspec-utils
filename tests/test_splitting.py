...
class TestSplittingWithoutHead:
    """Tests for stores without head() support."""

    def test_fallback_when_no_head(self):
        """Falls back to single get() when head() not available."""

        class NoHeadStore:
            """Store without head() method."""
        ...
        source = NoHeadStore({"file.txt": b"hello world"})
        splitter = SplittingReadableStore(source)

        # Should fall back to single get() since no head()
        result = splitter.get("file.txt")
        assert bytes(result.buffer()) == b"hello world"
